 ...
 uint8 distance3[80]=
 {
   180,180,180,180,180,159,159,159,156,156,
   155,151,150,148,156,155,153,153,151,150,
   148,147,145,143,143,141,139,137,136,134,
   132,130,130,128,126,124,122,120,119,119,
   117,114,113,111,109,109,107,104,103,101,
   99,97,95,95,93,91,89,87,86,84,
   83,81,80,78,76,74,72,71,70,69,
   ...
   for(j=0;j<nWidth;j++)
   {
    ...
    for(K=(nHeight-15);K>0;K--)                          //nHeight=80高度
    {
     ...
     if((//远处环岛预判断（1）
         ...
         &&((adc.l2.value+adc.l1.value+adc.r1.value+adc.r2.value)<900)
...
//           crossroad_ready = 0;
     ...
     if((circle_ready_l==1||circle_ready_r==1)&&
      ...
      ||(left_normal_count>=60&&right_normal_count>=60&&((adc.l2.value+adc.l1.value+adc.r1.value+adc.r2.value)<600))//大直道清零
      ||(left_lose_count>=55&&right_lose_count>=55&&((adc.l2.value+adc.l1.value+adc.r1.value+adc.r2.value)<600))
...
//              {
                  if((left_lose_count>right_lose_count)&&(left_lose_35up>=10||left_lose_15_30>=10))//左环
                  {
//                      BEEP_NO;
...
//              mid_point[i].x=mid_point[i-1].x-(all_lose_count+i)/2/4;
      ...
      {
          //出环岛大标志位
          if(angel_count>=210/*&&left_lose_count>=6*/)     {circle_out_flag2=1;}//小环岛
...
//              {mid_point[i].x=(left_point[i].x+right_point[i].x)/2;}
              ...
              {mid_point[i].x=left_point[i].x+distance3[i]+40;}
              else if(circle_ready_l==1)
              {mid_point[i].x=right_point[i].x-distance3[i]-40;}
              else
              {
                mid_point[i].x=(left_point[i].x+right_point[i].x)/2;
              }

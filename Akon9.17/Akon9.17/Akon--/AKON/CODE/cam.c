...
void Display_Pitcure(void)  //显示二值化图像和中线
{
    int i,j ;

    ips200_displayimage032(imageAdaptiveThreshold.data,imageAdaptiveThreshold.width,imageAdaptiveThreshold.height);

    for(i=length;i>15;i--)//画左边线点（蓝） 右边线点（绿） 中线（红）
    {

        ips200_drawpoint(left_point[i].x+4,nWidth/2-i,BLUE);
        ips200_drawpoint(left_point[i].x+5,nWidth/2-i,BLUE);
//        ips200_drawpoint(left_point[79-i].x+4,i,BLUE);

        ips200_drawpoint(right_point[i].x-4,nWidth/2-i,GREEN);
        ips200_drawpoint(right_point[i].x-5,nWidth/2-i,GREEN);
//        ips200_drawpoint(right_point[79-i].x-4,i,GREEN);

        ips200_drawpoint(mid_point[i].x,nWidth/2-i,RED);
        ips200_drawpoint(mid_point[i].x-1,nWidth/2-i,RED);
        ips200_drawpoint(mid_point[i].x+1,nWidth/2-i,RED);
    ...
    for(i=0;i<160;i++)
    {
        ips200_drawpoint(i,80-first_lose,RED);
    }
...
//////                show();
//////                for (int i = 0; i < imageAdaptiveThreshold.width / 2 - begin_x; i++) {
//////                    AT_IMAGE(&imageAdaptiveThreshold, (int) i, (int) begin_y) = 0;
//////                }
//////                for (int i = imageAdaptiveThreshold.width / 2 + begin_x; i < imageAdaptiveThreshold.width; i++) {
//////                    AT_IMAGE(&imageAdaptiveThreshold, (int) i, (int) begin_y) = 0;
//////                }
//////                
////////            }
//////            
//////             clear_image(&imageLine);
//////            
////////            adaptive_threshold(&imageRaw, &imageAdaptiveThreshold, 7, 2, 0, 255);

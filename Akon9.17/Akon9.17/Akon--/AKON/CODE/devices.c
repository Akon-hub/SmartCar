...
//  @param        time  在多长时间内
//  @return       void
//  Sample usage: Beep_DiDi(2, 150); 150ms内响2声
//----------------------------------------------------------------
...
void EncoderRead(void)
{
    ...
    wheel.left.last=0; // 自己写的，为了避免因为滤波清除不了last 2021.8.31
	//滤波
    ...
    wheel.right.last=0; // 自己写的，为了避免因为滤波清除不了last 2021.8.31
	//滤波
...
//	}
	...
	wheel.lastTotalSpeed = 0; //2021.8.31给上次的值给0，原来是wheel.now  因为 ：滤波事件
}
...
//----------------------------------------------------------------
//  @brief        定时器里面有关debug函数开关，默认为1ms，用除法自定义中断时间
//  @param        void
//  @return       void
